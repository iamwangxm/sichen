...
set rs=nothing
set conn=nothing
response.write "<script language=JavaScript>" & chr(13) & "alert('抱歉，普通用户预约上限为"&en_max2&"个，你可自行删除无效的学员信息来预约此学员；若您需要更多的库存容量，请升级成VIP用户，一次性付费100元（以后不用再续费），终身拥有"&en_vip_max2&"个学员库容量，付费方式请见会员中心左下方的“付款方式”，咨询电话：13412380803 QQ：27229133！ ');" & "history.back()" & "</script>" 
Response.End
end if
else
...
set rs=nothing
set conn=nothing
response.write "<script language=JavaScript>" & chr(13) & "alert('抱歉，您的"&en_vip_max2&"个预约学员库已满，你可自行删除无效的学员信息来预约此学员；若您需要更多的库存容量，请与管理员联系！电话：13412380803 QQ：27229133');" & "history.back()" & "</script>" 
Response.End
end if
end if
rs.close
...
if rs.recordcount<>0 then
rs.close
response.write "<script language=JavaScript>" & chr(13) & "alert('您已经预约过此学员！');" & "history.back()" & "</script>" 
Response.End
end if
rs.close

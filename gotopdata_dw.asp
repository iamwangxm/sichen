...
<%
dim vip
sql="select ID,VIP from  en_user where id="&clng(Request.Cookies("id"))
...
if vip=true then
	  ...
	  if request("G1")="提升教员信息" then
	  sql="update en_user set gotop=now() where id="&clng(Request.Cookies("id"))
	  conn.execute(sql)

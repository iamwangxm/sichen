...
<table width="90%" height="242" border="0" cellpadding="0" cellspacing="0" id="table5">
											<tr>
												<td height="40">
												<p align="center"><b>我曾预约过的学员</b></td>
											</tr>
											<tr>
												...
												<div align="center">
													<table border="0" width="620" cellspacing="0" cellpadding="0" id="table6">
														<%
														sql="select id,inid,addtime from enorder where enid="&clng(Request.Cookies("id"))&" order by id desc"
rs.open sql,conn,1,1



if rs.recordcount=0 then
           response.write "您暂时没有学员信息！<br>预约方法：找到需要的学员，在联系方式中点[预约此份家教]，即可预约指定的学员。"
           else
	rs.pagesize=10
	page=request("page")       
	if page="" then page=1       
...
%>	
						...
						<%=rs1("infudao")%></td>
															...
															<a onClick="{if(confirm('此操作将取消预约<<%=rs1("name")%>>学员，继续吗？')){return true;}return false;}" href="del_enorder.asp?id=<%=rs("id")%>&page=<%=page%>">
												  <img border="0" src="images/cancel.gif"  height="17"></a></td>
													  </tr>
														<%
rs.movenext
...
sub nextpage(x)      
	alldel.page.value=x        
	alldel.action="control.asp?file=enorder.asp"

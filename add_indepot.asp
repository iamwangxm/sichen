...
rs.close
sql="select id from indepot where inid="&clng(Request.Cookies("id"))& "and enid="&clng(request("enid"))
rs.open sql,conn,1,1
if rs.recordcount<>0 then
rs.close
response.write "<script language=JavaScript>" & chr(13) & "alert('您已经添加过此教员信息！');" & "history.back()" & "</script>" 
Response.End
end if
rs.close
sql="insert into indepot(inid,enid) values("&Request.Cookies("id")&","&request("enid")&")"
conn.execute(sql)
set rs=nothing
set conn=nothing

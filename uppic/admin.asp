...
%>
  <tr>
    <td width="880">
      <font size="1">序号：<%=rs("num")%>  上传日期：<%=rs("time")%> 用户ID：<% if rs("type")="教员" then %> <a href="../company.asp?id=<%=rs("id")%>" target="_blank"><%=rs("id")%></a><%else%><a href="../person.asp?id=<%=rs("id")%>" target="_blank"><%=rs("id")%></a><%end if%> 用户类型：<%=rs("type")%> <a href="kkk1.asp?id=<%=rs("num")%>" target="_blank">查看图片</a> <a href="delpic_admin.asp?id=<%=rs("num")%>" target="_blank">删除</a> </font></td>
  </tr> 
...
%>
<a href=admin.asp?page=<%=i%> target="_self"><%=i%></a>
<%next
...
<center>
  <table border="0" cellpadding="0" cellspacing="0"  width="748" height="27" >
    ...
    <b><a href="admin_dw.asp" target="_blank">教员用户</a> <a href="admin_gr.asp" target="_blank">学员用户</a></b></font>      </td>
    </tr>
  </table>

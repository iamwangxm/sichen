...
<table border="0" width="100%" cellspacing="0" cellpadding="4" bgcolor="#FFFFFF" style="border:1px solid #000000; " height="229">
	<tr>
		...
		<p align="center"><font size="2" color="#FFFFFF"><b>教员预约学员情况</b></font></td>
	</tr>
...
<%
sql="select id,inid,enid,addtime from enorder"
...
%>	
												<tr>
															...
															<a target =_blank  href="../teacher.asp?id=<%=rs2("id")%>">
															<%=rs2("name")%></a></td>
															...
															<a onClick="{if(confirm('此操作将删除<<%=rs1("name")%>>的学员收录，继续吗？')){return true;}return false;}" href="../del_enorder.asp?id=<%=rs("id")%>&page=<%=page%>">
...
sub nextpage(x)      
	alldel.page.value=x        
	alldel.action="admin_enorder.asp"

...
if rss.eof then
               ...
               town=rss("townname")
                ...
                set rss=nothing
                %>
                              当前设置：<font color=#ff0000><%=city%><%=town%><%=rs("xian")%></font><br>
...
set rs1_s=nothing
%></select><br>
            镇区：<select  size="1" name="town"  style=" font-size: 12px; background-color: #fff4e5" >
...
rs1_s.open sql2,conn,1,1
while not rs1_s.eof
%><option value="<%=rs1_s("TownNo")%>" <%if town1=rs1_s("TownNo") then%>selected<%end if%>><%=trim(rs1_s("townname"))%></option>
          <%
    rs1_s.movenext
wend
rs1_s.close
